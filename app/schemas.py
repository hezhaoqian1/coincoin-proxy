...
class AdminProviderChannelCreate(BaseModel):
    ...
    auth_style: str = Field(default="bearer", pattern=r"^(bearer|azure|api-key|x-api-key|anthropic_x_api_key)$")
...
class AdminProviderChannelUpdate(BaseModel):
    ...
    auth_style: Optional[str] = Field(default=None, pattern=r"^(bearer|azure|api-key|x-api-key|anthropic_x_api_key)$")
    status: Optional[str] = Field(default=None, pattern=r"^(active|disabled)$")
