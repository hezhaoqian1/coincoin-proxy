...
def _sanitize_encrypted_ids(payload: dict) -> None:
    """Replace ChatGPT-encrypted IDs in input items with plain ones.

    codex-zhuceji rotates ChatGPT tokens; encrypted call_ids / item ids
    generated by Token A cannot be decrypted by Token B, causing
    ``invalid_encrypted_content`` errors on multi-turn requests.
    """
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        item_id = item.get("id")
        if isinstance(item_id, str) and item_id.startswith(_ENCRYPTED_PREFIXES):
            item.pop("id", None)
        call_id = item.get("call_id")
        if isinstance(call_id, str) and call_id.startswith(_ENCRYPTED_PREFIXES):
            if call_id not in id_map:
                counter += 1
                id_map[call_id] = f"call_{counter:04d}"
            item["call_id"] = id_map[call_id]
        ...
        if isinstance(content, list):
            for part in content:
                if not isinstance(part, dict):
                    continue
                pid = part.get("id")
                if isinstance(pid, str) and pid.startswith(_ENCRYPTED_PREFIXES):
                    part.pop("id", None)
