...
def _sanitize_encrypted_ids(payload: dict) -> None:
    ...
    codex-zhuceji rotates ChatGPT tokens; encrypted values (gAAA…/gBAA…)
    generated by Token A cannot be decrypted by Token B.  The ChatGPT
    backend scans the ENTIRE request body for encrypted content, so we
    must walk every nested dict/list and sanitize every matching string.
    """
    id_map: dict[str, str] = {}
    counter = [0]

    def _clean(key: str, val: str) -> str | None:
        """Return replacement value, or None to signal 'delete this key'."""
        if key == "call_id":
    ...
    def _walk(obj):
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                if isinstance(v, str) and v.startswith(_ENCRYPTED_PREFIXES):
                    replacement = _clean(k, v)
