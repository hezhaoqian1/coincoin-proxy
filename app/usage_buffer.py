...
class UsageBuffer:
    ...
    def __init__(self) -> None:
        ...
        # 每日统计: (user_id, date) -> {input_tokens, output_tokens, requests, cost_cents}
        self._daily: Dict[Tuple[str, date], Dict[str, int]] = defaultdict(
            lambda: {"input_tokens": 0, "output_tokens": 0, "requests": 0, "cost_cents": 0}
        )
        # 用户累计: user_id -> {input_tokens, output_tokens, cost_cents}
        self._usage_by_user: Dict[str, Dict[str, int]] = defaultdict(
            lambda: {"input_tokens": 0, "output_tokens": 0, "cost_cents": 0}
        )
    ...
    async def add(
        ...
        async with lock:
            ...
            bucket["cost_cents"] += int(cost_cents)
            ...
            user_bucket["cost_cents"] += int(cost_cents)
    ...
    async def get_pending_cost(self, user_id: str) -> int:
        ...
        """
        usage = self._usage_by_user.get(user_id, {})
        return int(usage.get("cost_cents", 0))
        ...
        async with self._global_lock:
            ...
            try:
                for key, stats in daily.items():
                    ...
                    bucket["cost_cents"] += int(stats.get("cost_cents", 0))
                for user_id, usage in usage_by_user.items():
                    ...
                    user_bucket["cost_cents"] += int(usage.get("cost_cents", 0))
            finally:
                for lock in self._locks:
                    lock.release()
    ...
    try:
        async with SessionLocal() as session:
            ...
            for user_id, usage in usage_by_user.items():
                ...
                cost_cents = int(usage.get("cost_cents", 0))
            ...
            for (user_id, day), stats in daily.items():
                ...
                cost_cents = int(stats.get("cost_cents", 0))
                total_tokens = input_tokens + output_tokens
