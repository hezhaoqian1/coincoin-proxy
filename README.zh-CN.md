...
- 同步图片请求默认每 15 秒发送一次 JSON 合法空白，避免上游生成期间因连接长期无数据而触发 CDN 524；空白不会影响最终 JSON 解析
- 第一次保活写出后 HTTP 状态会固定为 `200`，晚到的上游错误通过最终 OpenAI-compatible JSON error body 表达；需要短请求或严格错误状态时使用异步图片任务
...
创建请求会立即返回 `202` 和任务 `id`，再使用下面的查询接口轮询。异步不会缩短生图时间，它只避免客户端长期占用一个 HTTP 连接。
