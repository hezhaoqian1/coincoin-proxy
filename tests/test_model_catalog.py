...
def _legacy_text_model(model_id: str) -> dict:
    ...
    if model_id in {"gpt-6", "gpt-6-astra", "gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"}:
        model["pricing"] = {"cache_read_multiplier": 0.1, "cache_creation_multiplier": 1.25}
    if model_id in {"gpt-6", "gpt-6-astra"}:
        model["metadata"] = {
            "display_name": "GPT-6 Astra",
            "version": "gpt-6",
            "description": "OpenAI GPT-6 Astra general-purpose model.",
            "context_length": 1050000,
            "max_completion_tokens": 128000,
            "supported_parameters": ["tools", "vision", "reasoning_effort"],
        }
    if model_id == "gpt-5.4":
        model["metadata"] = {
...
class ModelCatalogTests(unittest.TestCase):
    ...
    def test_runtime_alias_override_changes_upstream_without_editing_catalog(self) -> None:
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as override_file:
            json.dump(
                {
                    "aliases": {
                        "gemini-fast": {
        ...
        try:
            settings.model_alias_overrides_path = override_path
            registry._initialized = False
            registry.init_from_settings()
    ...
    def test_gpt_6_astra_uses_official_pricing_and_metadata(self) -> None:
        for model_id in ("gpt-6", "gpt-6-astra"):
            model = registry.get_public_model(model_id)
            self.assertIsNotNone(model)
            self.assertEqual(model.price_input_per_million, 1000)
            self.assertEqual(model.price_output_per_million, 5000)
            self.assertEqual(model.effective_cached_input_per_million, 100.0)
            self.assertEqual(model.effective_cache_creation_input_per_million, 1250.0)
            self.assertEqual(model.metadata["context_length"], 1050000)
            self.assertEqual(model.metadata["max_completion_tokens"], 128000)
