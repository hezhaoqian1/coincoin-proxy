...
class GatewayCatalogSyncTests(unittest.TestCase):
    ...
    def test_gpt_6_astra_uses_official_model_and_cache_pricing(self) -> None:
        public_models = {
            item["id"]: item
            for item in (self.catalog.get("models") or [])
            if isinstance(item, dict) and item.get("id")
        }
        for public_id in ("gpt-6", "gpt-6-astra"):
            with self.subTest(model=public_id):
                model = public_models[public_id]
                self.assertEqual(model.get("provider_model"), "gpt-6-astra")
                self.assertEqual(model.get("routing_mode"), "legacy_auto")
                self.assertEqual(model.get("delivery_lane"), "legacy")
                self.assertEqual(_placeholder_default(model.get("price_input_per_million")), "1000")
                self.assertEqual(_placeholder_default(model.get("price_output_per_million")), "5000")
                self.assertEqual((model.get("pricing") or {}).get("cache_read_multiplier"), 0.1)
