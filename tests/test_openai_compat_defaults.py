...
def _legacy_text_model(model_id: str) -> dict:
    ...
    if model_id in {"gpt-6", "gpt-6-astra", "gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"}:
...
class OpenAICompatDefaultsTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self._workbench_auth_patchers = []

        async def delegate_openai_workbench_auth(request, db):
            return await openai_module.authorize_request(request, db)

        async def delegate_proxy_workbench_auth(request, db):
            return await proxy_module.authorize_request(request, db)

        self._workbench_auth_patchers.append(
            patch.object(openai_module, "authorize_workbench_request", delegate_openai_workbench_auth)
        )
        self._workbench_auth_patchers.append(
            patch.object(proxy_module, "authorize_workbench_request", delegate_proxy_workbench_auth)
        )
        for patcher in self._workbench_auth_patchers:
            patcher.start()
    ...
    def _add_claude_code_model(self, *, with_route: bool = False) -> None:
        catalog = json.loads(settings.model_catalog_json)
        ...
        registry.init_from_settings()
        if with_route:
            self._install_claude_code_route()

    def _install_claude_code_route(self) -> None:
        channel_router.set_snapshot(
            [
                ProviderChannelSnapshot(
                    channel_id="ch_claude_code_test",
            ...
            [
                ModelChannelRouteSnapshot(
                    route_id="mcr_claude_code_responses",
    ...
    def _add_grok_model(self, public_model_id: str = "grok-build") -> None:
        catalog = json.loads(settings.model_catalog_json)
        catalog.setdefault("models", []).append(
            {
                "id": public_model_id,
                ...
                "price_input_per_million": 500,
                "price_output_per_million": 3000,
                "billable_sku": f"xai-{public_model_id}-text",
            }
        )
        settings.model_catalog_json = json.dumps(catalog)
        registry._initialized = False
    ...
    async def test_grok_build_responses_preserves_flat_function_tools(self) -> None:
        ...
        self.assertEqual(usage_kwargs["model"], "grok-build")
        self.assertEqual(usage_kwargs["customer_model_alias"], "grok-build")
    ...
    async def test_grok_4_5_responses_stream_preserves_public_model_and_usage_identity(self) -> None:
        self._add_grok_model("grok-4.5")
        upstream_client = _RecordingStreamClient(
            [
                _FakeEventStreamResponse(
                    [
                        'data: {"type":"response.created","response":{"id":"resp_grok_stream","status":"in_progress","model":"grok-4.5-build-free","output":[]}}',
                        "",
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            with patch.object(proxy_module, "authorize_request", AsyncMock(return_value=self.fake_user)), patch.object(
                ...
                response = await client.post(
                    "/v1/responses",
                    headers={"Authorization": "Bearer sk_cc_test"},
                    json={"model": "grok-4.5", "input": "Reply exactly: GROK_STREAM_OK", "stream": True},
                )

        self.assertEqual(response.status_code, 200, response.text)
        self.assertIn("GROK_STREAM_OK", response.text)
        self.assertIn('"model":"grok-4.5"', response.text.replace(" ", ""))
        self.assertNotIn("grok-4.5-build-free", response.text)
        self.assertEqual(upstream_client.calls[0]["url"], "https://grok.example/v1/responses")
        self.assertEqual(upstream_client.calls[0]["json"]["model"], "grok-4.5")
        self.assertEqual(upstream_client.calls[0]["headers"]["authorization"], "Bearer grok-key")
        add_usage.assert_awaited_once()
        ...
        self.assertEqual(usage_kwargs["model"], "grok-4.5")
        self.assertEqual(usage_kwargs["provider_model"], "grok-4.5")
    ...
    async def test_grok_build_chat_converts_vision_input_for_responses(self) -> None:
                ...
                _FakeUpstreamResponse(
                    {
                        ...
                        "output": [
                            {
                                "type": "message",
                                "content": [{"type": "output_text", "text": "GROK_VISION_OK"}],
                            }
                        ],
        ...
        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(response.json()["choices"][0]["message"]["content"], "GROK_VISION_OK")
        content = upstream_client.calls[0]["json"]["input"][0]["content"]
        self.assertEqual(content[0], {"type": "input_text", "text": "Inspect the image"})
        self.assertEqual(content[1], {"type": "input_image", "image_url": image_url})

    async def test_grok_build_responses_preserves_function_call_output_round_trip(self) -> None:
        self._add_grok_model()
        upstream_client = _RecordingClient(
            [
                _FakeUpstreamResponse(
                    {
                        "id": "resp_grok_tool_result",
                        "model": "grok-4.5-build-free",
                        "status": "completed",
                        "output": [
                            {
                                "type": "message",
                                "content": [{"type": "output_text", "text": "GROK_BUILD_TOOL_LOOP_OK"}],
                            }
                        ],
        ...
        tool_result = {
            ...
            "call_id": "call_grok_read",
            "output": "GROK_BUILD_TOOL_LOOP_OK",
        }
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            with patch.object(proxy_module, "authorize_request", AsyncMock(return_value=self.fake_user)), patch.object(
                ...
                response = await client.post(
                    "/v1/responses",
                    headers={"Authorization": "Bearer sk_cc_test"},
                    json={"model": "grok-build", "input": [tool_result]},
                )

        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(upstream_client.calls[0]["json"]["input"], [tool_result])
        self.assertEqual(response.json()["model"], "grok-build")
        self.assertEqual(
            response.json()["output"][0]["content"][0]["text"],
            "GROK_BUILD_TOOL_LOOP_OK",
        )
    ...
    async def test_responses_provider_channel_can_fallback_from_empty_output_to_403_to_success(self) -> None:
        channel_router.set_snapshot(
            [
                ProviderChannelSnapshot(
                    channel_id="ch_responses_primary",
        ...
        upstream_client = _RecordingClient(
            [
                ...
                _FakeUpstreamResponse(
                    {
                        "id": "resp_responses_third_success",
                        "output": [
                            {
                                "type": "message",
                                "content": [{"type": "output_text", "text": "OK"}],
        ...
        add_usage.assert_awaited_once()
        usage_kwargs = add_usage.await_args.kwargs
        self.assertEqual(usage_kwargs["route_reason"], "channel_fallback:403")
        self.assertEqual(usage_kwargs["channel_id"], "ch_responses_third")
        self.assertEqual(usage_kwargs["fallback_from_channel_id"], "ch_responses_primary,ch_responses_second")
        self.assertEqual(usage_kwargs["route_attempt"], 2)
    ...
    async def test_responses_upstream_balance_error_is_reported_as_service_unavailable(self) -> None:
                ...
                _FakeUpstreamResponse(
                    {
                        ...
                        "error": {
                            "type": "new_api_error",
                            "message": "用户额度不足, 剩余额度: $ 14.518750, 最低保留额度: $ 9.000000 (request id: req_secret)",
                        },
                    },
                    status_code=403,
        ...
        self.assertEqual(response.status_code, 503, response.text)
        payload = response.json()
        self.assertEqual(payload["error"]["code"], "service_quota_unavailable")
        self.assertEqual(payload["error"]["type"], "server_error")
        self.assertEqual(payload["error"]["message"], "当前服务额度不足，请联系管理员处理。")
        self.assertNotIn("14.518750", response.text)
    ...
    async def test_responses_claude_code_alias_adds_prompt_cache_key(self) -> None:
        self._add_claude_code_model(with_route=True)
    ...
    async def test_responses_gpt_5_5_adds_prompt_cache_key_and_retention(self) -> None:
        fake_user = SimpleNamespace(id="u_test", _api_key_id="k_gpt55_resp")
        upstream_client = _RecordingClient(
            [
                _FakeUpstreamResponse(
                    {
    ...
    async def test_responses_gpt_5_5_preserves_client_prompt_cache_key(self) -> None:
        ...
        self.assertEqual(response.status_code, 200, response.text)
        request_json = upstream_client.calls[0]["json"]
        self.assertEqual(request_json.get("prompt_cache_key"), "client-session-key")
        self.assertEqual(request_json.get("prompt_cache_retention"), "24h")
    ...
    async def test_chat_provider_channel_can_fallback_across_multiple_routes(self) -> None:
        ...
        upstream_client = _RecordingClient(
            [
                _FakeUpstreamResponse(
                    {"error": {"message": "primary failed", "type": "server_error", "code": "500"}},
                    status_code=500,
                ),
                _FakeUpstreamResponse(
                    {
        ...
        usage_kwargs = add_usage.await_args.kwargs
        self.assertEqual(usage_kwargs["route_reason"], "channel_fallback:upstream_empty_response")
        self.assertEqual(usage_kwargs["channel_id"], "ch_chat_third")
        self.assertEqual(usage_kwargs["fallback_from_channel_id"], "ch_chat_primary,ch_chat_second")
        self.assertEqual(usage_kwargs["route_attempt"], 2)
    ...
    async def test_models_endpoint_returns_curated_metadata(self) -> None:
        ...
        self.assertEqual(models_by_id["gpt-6"]["coincoin_price_input_per_million"], 1000)
        self.assertEqual(models_by_id["gpt-6"]["coincoin_price_output_per_million"], 5000)
        self.assertEqual(models_by_id["gpt-6"]["coincoin_price_cache_creation_input_per_million"], 1250.0)
        self.assertEqual(models_by_id["gpt-6-astra"]["coincoin_price_input_per_million"], 1000)
